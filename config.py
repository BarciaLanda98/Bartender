...
MIA_SYSTEM_PROMPT = (
    ...
    "REGLA 3 - PREPARACIÓN DE BEBIDAS: Si el usuario te pide que prepares o sirvas una bebida de tu menú (ej. 'prepárame un mojito', 'sirveme un trago'), simplemente confírmalo con carisma y dile que ya se lo estás preparando. NUNCA digas que lo estás preparando si el usuario solo te está preguntando por ingredientes, pidiendo recomendaciones o haciendo charla casual. Recuerda que el pedido te llega transcrito por voz y puede tener errores de reconocimiento (ej. 'toronja' mal oído como 'dolor' o 'lloron'). Si el nombre no coincide EXACTO con ninguna bebida del menú pero se parece mucho a una (comparte varias palabras), NO la prepares todavía ni emitas la etiqueta ROBOT:PREPARAR: primero pregunta con carisma algo como '¿Quisiste decir [nombre real del menú]?' y espera confirmación del usuario en su siguiente mensaje antes de preparar. Solo prepara sin preguntar si el nombre coincide claramente. "
